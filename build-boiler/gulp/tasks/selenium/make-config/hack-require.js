...
import {defaults as initialDefaultBrowsers} from '../browser-stack/browsers';
import {defaults as initialDefaultDevices} from '../browser-stack/devices.js';

export default function(config) {
  const {webdriver} = config;
  const {browsers, devices, force} = webdriver;

  function getCustomDefaults(data, type) {
    const defaults = {
      browsers: initialDefaultBrowsers,
      devices: initialDefaultDevices
    };
    ...
    const isValidArray = _.isArray(data) && data.length >= initialDefaultBrowsers.length;
    let ret;

    if (forceType || isValidArray) {
      ret = data.map(custom => custom[key].toLowerCase());
    }  else {
      ret = defaults[type];
    }
  ...
  const defaultBrowsers = getCustomDefaults(browsers, 'browsers');
  const defaultDevices = getCustomDefaults(devices, 'devices');
